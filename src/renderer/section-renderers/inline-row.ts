...
export function renderInlineRow(
  ...
  const childrenHtml = config.children
    .map((child, index) => {
      ...
      return `<div class="inline-row-item" style="flex: ${weight} 1 ${flexBasis}; min-width: 0;">${childHtml}</div>`
    })
    .join('\n')
  
  return `<div class="print-section inline-row" style="display: flex; gap: ${gap}; align-items: flex-start;">
${childrenHtml}
</div>`
}
